...
public class MatchController {
    ...
    public MatchController(MatchService matchService, AuthService authService, org.springframework.messaging.simp.SimpMessagingTemplate template) {
     ...
     *
     * Remove user from match and reset currentMatchId; handle host transfer/room cleanup.
     */
    @org.springframework.web.bind.annotation.DeleteMapping("/api/matches/{id}/leave")
    public ResponseEntity<ApiResponse<Void>> leaveMatch(
        ...
        try {
            com.nhomgame.domain.match.Match updatedMatch = matchService.leaveMatch(matchId, user.getId());
            template.convertAndSend("/topic/match." + matchId, new WsEvent<>("player_left", user.getId()));
            if (updatedMatch == null) {
                template.convertAndSend("/topic/match." + matchId, new WsEvent<>("match_closed", matchId));
            }
